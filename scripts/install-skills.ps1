...
foreach ($plannedInstall in $plannedInstalls) {
    ...
    $installResult = Copy-SkillInstallation -Source $plannedInstall.SourcePath -TargetRoot $plannedInstall.TargetRoot -Name $plannedInstall.Name -Metadata $metadata -Force:$Force
