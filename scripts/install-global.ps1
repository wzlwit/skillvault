...
$bootstrapSkillNames = @(Read-BootstrapSkillNames -RepositoryRoot $RepoRoot)
...
$updateTargets = @($plannedInstalls | Where-Object Action -NE skip | ForEach-Object { $_.TargetPath })
$updateTargets += @($legacySkillEntries | ForEach-Object {
    $legacy = Join-Path $globalSkillsRoot ($_.Split(':', 2)[0])
    if (Test-Path -LiteralPath $legacy -PathType Container) { $legacy }
})
$updateLease = if ($updateTargets.Count) { Enter-SkillUpdateOwnership -Paths $updateTargets -ConfirmStopped:$ConfirmStopped } else { $null }
try {
foreach ($plannedInstall in $plannedInstalls) {
    ...
    $installResult = Copy-SkillInstallation -Source $plannedInstall.SourcePath -TargetRoot $globalSkillsRoot -Name $plannedInstall.Name -Metadata $metadata -Force:$Force -OwnershipLease $updateLease
