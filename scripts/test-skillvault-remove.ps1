...
$removeScript = Join-Path $PSScriptRoot '..\skills\core\skillvault-authoring\scripts\skillvault-remove.ps1'
...
try {
    ...
    $otherRepositoryPath = Join-Path $fixtureRoot 'other-repo/skills/testing/alpha'
    ...
    foreach ($unsafePath in @('../other-repo', 'skills/public/testing/alpha')) {
        $catalog[0].path = $unsafePath
        ConvertTo-Json -InputObject $catalog -Depth 5 | Set-Content -LiteralPath $catalogPath -Encoding utf8
    ...
    $catalog[0].path = 'skills/testing/alpha'
    ...
    $bootstrapSelectionPath = Join-Path $repositoryRoot 'scripts/bootstrap-skills.json'
    New-Item -ItemType Directory -Path (Split-Path -Parent $bootstrapSelectionPath) -Force | Out-Null
    '["alpha"]' | Set-Content -LiteralPath $bootstrapSelectionPath -Encoding utf8
    Assert-Throws { & $removeScript -Name alpha -RepoRoot $repositoryRoot -Force } '*registered for bootstrap*'
    '["beta"]' | Set-Content -LiteralPath $bootstrapSelectionPath -Encoding utf8
    $null = & $removeScript -Name alpha -RepoRoot $repositoryRoot
    Remove-Item -LiteralPath $bootstrapSelectionPath
    $bootstrapPath = Join-Path $repositoryRoot 'scripts/install-global.ps1'
    '' | Set-Content -LiteralPath $bootstrapPath -Encoding utf8
    Assert-Throws { & $removeScript -Name alpha -RepoRoot $repositoryRoot -Force } '*Missing bootstrap selection*'
    Remove-Item -LiteralPath $bootstrapPath
    $shellBootstrapPath = Join-Path $repositoryRoot 'scripts/install-global.sh'
    '' | Set-Content -LiteralPath $shellBootstrapPath -Encoding utf8
    Assert-Throws { & $removeScript -Name alpha -RepoRoot $repositoryRoot -Force } '*Missing bootstrap selection*'
