...
export function validateRuntimeInterfaces(manifest) {
  const checkVersions = (interfaces, label) => {
    assert.ok(interfaces && typeof interfaces === 'object' && !Array.isArray(interfaces), `${label}: expected an interface mapping`);
    for (const [name, version] of Object.entries(interfaces)) {
      assert.ok(namePattern.test(name) && Number.isSafeInteger(version) && version > 0, `${label}: invalid interface ${name}`);
    }
  };
  if (own(manifest, 'runtimeInterfaces')) checkVersions(manifest.runtimeInterfaces, manifest.name);
  if (own(manifest, 'requiredInterfaces')) {
    assert.ok(manifest.requiredInterfaces && typeof manifest.requiredInterfaces === 'object' && !Array.isArray(manifest.requiredInterfaces), `${manifest.name}: expected dependency interface requirements`);
    for (const [dependency, interfaces] of Object.entries(manifest.requiredInterfaces)) {
      assert.ok(namePattern.test(dependency) && Array.isArray(manifest.dependencies) && manifest.dependencies.includes(dependency), `${manifest.name}: interface requirements need a declared dependency ${dependency}`);
      checkVersions(interfaces, `${manifest.name}/${dependency}`);
    }
  }
}
...
export function validateRepository(root) {
  ...
  for (const entry of catalog) {
    ...
    assert.ok(parts.length === 3 && parts[0] === 'skills' && namePattern.test(parts[1]) && parts[2] === entry.name,
      `${entry.name}: expected skills/<category>/<name>`);
    assert.ok(!paths.has(entry.path), `${entry.name}: duplicate catalog path`);
    paths.add(entry.path);
    const directory = resolveInside(root, entry.path);
    resolveInside(fs.realpathSync(path.join(root, 'skills')), fs.realpathSync(directory));
  ...
  for (const [name, manifest] of manifests) {
    for (const [dependency, interfaces] of Object.entries(manifest.requiredInterfaces ?? {})) {
      for (const [contract, version] of Object.entries(interfaces)) {
        assert.equal(manifests.get(dependency)?.runtimeInterfaces?.[contract], version, `${name}: incompatible source dependency ${dependency}/${contract}`);
      }
    }
  }
