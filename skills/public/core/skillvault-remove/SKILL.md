---
name: skillvault-remove
description: Remove skill source folders and catalog entries from a selected local SkillVault repository after confirmation. Use for /skillvault-remove, /sv-remove, /skv-remove, or "remove skill from source repo"; use skillvault-uninstall for installed copies.
metadata:
  author: wzlwit
  version: "1.0.0"
argument-hint: "<skill-name[,skill-name...]> [repo-path]"
---
...
- `repo-path`: optional local source checkout. An explicit path must contain `catalog.json`
  and `skills/public/`; an invalid path is an error, not permission to choose another repo.
...
5. Apply only the approved active-documentation cleanup. Do not uninstall copies, edit other
   ...
   offer `/sv-uninstall` separately if installed copies should also be removed.
...
`skillvault-install` skill's shared file helpers. Preview is the default:

```powershell
~/.copilot/skills/skillvault-remove/scripts/skillvault-remove.ps1 -Name architecture-decision-records -RepoRoot C:\repos\skillvault
```
...
- Delete only the selected catalog-backed `skills/public/<category>/<name>` directories.
