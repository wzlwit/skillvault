...
$helperPath = Join-Path $PSScriptRoot '..\..\skillvault-installation\scripts\skill-files.ps1'
...
$removals = @(foreach ($skillName in $requestedNames) {
    ...
    if ($segments.Count -ne 3 -or $segments[0] -cne 'skills' -or
        $segments[1] -cnotmatch '^[a-z0-9]+(-[a-z0-9]+)*$' -or $segments[2] -cne $skillName) {
...
if ((Test-Path -LiteralPath $bootstrapSelectionPath) -or (Test-Path -LiteralPath $bootstrapPath) -or (Test-Path -LiteralPath $shellBootstrapPath)) {
    $bootstrapNames = @(Read-BootstrapSkillNames -RepositoryRoot $repositoryRoot)
}
foreach ($skillName in $requestedNames) {
    if ($skillName -cin $bootstrapNames) {
        throw "Skill '$skillName' is registered for bootstrap. Update scripts/bootstrap-skills.json before removal."
    }
}
